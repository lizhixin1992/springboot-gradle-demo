...
public class RedisUtils {
    ...
    public String getSetCollections(String key) {
        String result = JSONObject.toJSONString(redisTemplate.opsForSet().members(key));
    ...
    public String getMapCollections(String key) {
        return JSONObject.toJSONString(redisTemplate.opsForHash().entries(key));
    }
    ...
    public String getListStartEnd(String key, int start, int end) {
        String result = JSONObject.toJSONString(redisTemplate.opsForList().range(key, start, end));
        return result.substring(1, result.length() - 1);
    }
